...
Thread::Thread(char* threadName, int threadID)
{
    ...
    space = NULL;
    DEBUG(dbgThread, "Thread created. ID: [" << threadID << "], Name: [" << threadName << "]");
...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    StackAllocate(func, arg); //func = InitialPCState
...
Thread::Yield ()
{
    ...
    ASSERT(this == kernel->currentThread); //確保threads一致姓
    ...
    if (nextThread != NULL) {
    //MP3
    double nowRunningTime = kernel->currentThread->getRunningBrustTime() + kernel->stats->totalTicks - kernel->currentThread->getInRunningState();
    //cout << "in running state: " << kernel->currentThread->getInRunningState() << endl;
    //DEBUG(dbgThread, "in running state: " << kernel->currentThread->getInRunningState());
    kernel->currentThread->setRunningBurstTime(nowRunningTime);


    DEBUG('z', "[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getName() << "] is now selected for execution, thread [" << this->getName() << "] is replaced, and it has executed [" << this->getRunningBrustTime() << "] ticks");

	kernel->scheduler->ReadyToRun(this);
    nextThread->setInRunningState(kernel->stats->totalTicks); //MP3
...
Thread::Sleep (bool finishing)
{
    ...
    //MP3 ->注意!這裡仍有可能出現通往terminate的  不只有waiting!
    // kernel->currentThread->setTotalTime(kernel->currentThread->getTotalTime() + kernel->currentThread->getRunningBrustTime());

    double nowRunningTime = kernel->currentThread->getRunningBrustTime() + kernel->stats->totalTicks - kernel->currentThread->getInRunningState();
    double oldBurst = kernel->currentThread->getBurstTime(), t = nowRunningTime;
    double newBurst = t * 0.5 + oldBurst * 0.5;
    
    if (!finishing) DEBUG('z', "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << kernel->currentThread->getName() << "] update approximate burst time, from: [" << oldBurst << "], add [" << t << "], to [" << newBurst << "]");
    
    kernel->currentThread->setBurstTime(newBurst);
    kernel->currentThread->setRunningBurstTime(0);
    // kernel->currentThread->setTotalTime(0);
    ...
    DEBUG('z', "[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getName() << "] is now selected for execution, thread [" << kernel->currentThread->getName() << "] is replaced, and it has executed [" << t << "] ticks");
