...
int Kernel::Exec(char* name, int fileId)
{
	t[threadNum] = new Thread(name, threadNum);
	t[threadNum]->space = new AddrSpace();

    //MP3
    if(priority[threadNum] > 0){
        t[threadNum]->setPriority(priority[fileId]);
    }
    else {
        t[threadNum]->setPriority(0);
    }
    DEBUG(dbgThread, "thread: [" << t[threadNum]->getName() << "], priority: [" << t[threadNum]->getPriority() << "]");
